...
export function ExportModal({
  ...
  const sanitizeDownloadFilename = (input: string) => {
    ...
    return (cleaned.slice(0, 100) || "project").concat(`.${selectedFormat}`);
  };
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const serverFilename =
        ...
        sanitizeDownloadFilename(projectTitle);
  ...
  const renderFormatIcon = (format: ExportFormat) => {
    if (format === "txt") {
      return (
        <svg
          className="w-8 h-8"
    ...
    return (
      <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24">
        ...
        <path
          ...
          strokeLinejoin="round"
          strokeWidth={1.5}
          d="M14 2v6h6M8 13l2 5 2-5 2 5 2-5"
        />
      </svg>
    );
  };
      ...
      <DialogContent className="sm:max-w-md bg-white dark:bg-[#111418] border-gray-200 dark:border-gray-800">
        ...
        <DialogHeader>
          ...
          <DialogDescription className="text-center text-gray-500 dark:text-gray-400">
            Download your novel in various formats
          </DialogDescription>
        </DialogHeader>
